...
import { setStudents, fetchStudents } from '../../app/redux/actions/students'
  ...
  describe('Redux', () => {
    ...
    describe('reducer', () => {
      ...
      it('*** returns the initial state by default', () => {
        ...
        expect(newState.studentState.students.length).to.equal(0);
      })
      ...
      it('reduces on SET_STUDENTS action', () => {
        ...
        expect(newState.studentState.students).to.be.deep.equal(students)
        expect(newState.studentState.students).to.not.be.equal(prevState.students)
      })
    })
  })
  ...
  describe('Sequelize Model', () => {
    ...
    it('*** email must be a valid email', async () => {
      const student = Student.build({
        firstName: 'Sally',
        lastName: 'Ride',
        email: 'sallyridenasa.gov',
      })
      try {
        await student.validate()
        throw Error('validation should have failed with non valid email')
      } catch (err) {
