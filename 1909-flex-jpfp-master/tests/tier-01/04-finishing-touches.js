...
describe('Tier One: Final Touches', () => {
 ...
 describe('React-Redux', () => {
    ...
    it('initializes campuses and students from the server when the app first loads', async () => {
      ...
      expect(reduxStateBeforeMount.campusState.campuses).to.deep.equal([])
      expect(reduxStateBeforeMount.studentState.students).to.deep.equal([])
      mount(
        <Provider store={store}>
      ...
      expect(reduxStateAfterMount.campusState.campuses).to.deep.equal(campuses)
      expect(reduxStateAfterMount.studentState.students).to.deep.equal(students)
    })
